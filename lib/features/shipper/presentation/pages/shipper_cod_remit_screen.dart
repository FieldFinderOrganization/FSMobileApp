...
/// Màn shipper nộp tiền COD: hiện QR PayOS, poll tới khi BE xác nhận đã giảm nợ ví.
class ShipperCodRemitScreen extends StatefulWidget {
  final ShipperCodRemitModel remit;
  final ShipperRemoteDataSource ds;
  final VoidCallback? onCredited;

  const ShipperCodRemitScreen({
    super.key,
    required this.remit,
    required this.ds,
    this.onCredited,
  ...
  @override
  State<ShipperCodRemitScreen> createState() => _ShipperCodRemitScreenState();
}

class _ShipperCodRemitScreenState extends State<ShipperCodRemitScreen> {
  Timer? _pollTimer;
  bool _done = false;
  ...
  Future<void> _poll() async {
    if (_done) return;
    try {
      final status = await widget.ds.pollCodRemitStatus(widget.remit.remitId);
  ...
  Widget _notice() => Container(
        ...
        child: Row(
          children: [
            ...
            Expanded(
              child: Text(
                'Quét mã để chuyển khoản nộp tiền hàng COD. Công nợ ví tự giảm sau khi hệ thống xác nhận giao dịch.',
                style: TextStyle(
                    fontSize: 12,
  ...
  Widget _qr() => Container(
        ...
        child: Column(
          children: [
            if (widget.remit.qrCode != null)
              QrImageView(
                data: widget.remit.qrCode!,
            ...
            const Text('Quét mã để nộp tiền COD',
  ...
  Widget _details() => Container(
        ...
        child: Column(
          children: [
            ...
            _row('Ngân hàng', 'MB'),
            const Divider(height: 24),
            _row('Số tiền nộp', formatVnd(widget.remit.amount),
      ...
      builder: (d) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            const Text('NỘP TIỀN THÀNH CÔNG',
                style: TextStyle(fontWeight: FontWeight.w900)),
            const SizedBox(height: 8),
            Text('Đã giảm ${formatVnd(widget.remit.amount)} công nợ COD.',
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed: () {
                  ...
                  Navigator.pop(context); // đóng màn nộp
                },
