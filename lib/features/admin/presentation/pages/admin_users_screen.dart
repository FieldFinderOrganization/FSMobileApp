...
class _AdminUsersScreenState extends State<AdminUsersScreen> {
  ...
  Future<void> _exportPdf() async {
    ...
    try {
        ...
        pw.MultiPage(
          ...
          build: (ctx) => [
            ...
            if (stats != null) ...[
              ...
              pw.Bullet(
                text: 'Tổng người dùng: ${stats.total}',
                style: pw.TextStyle(font: font),
              ),
              ...stats.byStatus.map(
                (s) => pw.Bullet(
      ...
      final fileName =
          'admin_nguoi_dung_${DateFormat('yyyyMMdd').format(now)}.pdf';
        ...
        slivers: [
          ...
          if (_loading)
            const SliverFillRemaining(
              child: Center(
                child: CircularProgressIndicator(color: Color(0xFF4454A0)),
              ),
            )
          else if (_error != null)
            ...
            if (stats != null)
              SliverToBoxAdapter(child: _buildDonutCard(stats, total)),
                ...
                Expanded(
                  child: Column(
                    children: List.generate(roles.length, (i) {
                      ...
                      final pct = total > 0
                          ? (r.count / total * 100).toStringAsFixed(1)
                          : '0';
                      return Padding(
            ...
            child: Column(
              children: [
                ...users.asMap().entries.map(
                  (e) => _buildUserRow(e.value, e.key == users.length - 1),
                ),
                if (users.isEmpty)
                  Padding(
  ...
  Color _avatarColor(String name) {
    ...
    final idx =
        name.codeUnits.fold(0, (a, b) => a + b) % _kAvatarPalette.length;
