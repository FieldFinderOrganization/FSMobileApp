...
class _AdminOrdersScreenState extends State<AdminOrdersScreen> {
  ...
  void _showFilterSheet() {
    ...
    final minCtrl = TextEditingController(
      text: _filterMinAmount?.toStringAsFixed(0) ?? '',
    );
    final maxCtrl = TextEditingController(
      text: _filterMaxAmount?.toStringAsFixed(0) ?? '',
    );
                      ...
                      Wrap(
                        ...
                        children:
                            [
                              ...
                              {
                                'label': 'Khách đặt nhiều nhất',
                                'value': 'customer_most_orders',
                              },
                            ].map((opt) {
  ...
  Future<void> _exportPdf() async {
    ...
    try {
      ...
      pdf.addPage(
        pw.MultiPage(
          ...
          build: (ctx) => [
            ...
            pw.Text(
              'Số bản ghi xuất: ${items.length} / $total',
              style: pw.TextStyle(font: font),
            ),
      ...
      final fileName =
          'admin_don_hang_${DateFormat('yyyyMMdd').format(now)}.pdf';
  ...
  Widget _buildDonut() {
    final values = _stats
        .map((s) => ((s['count'] as num?) ?? 0).toDouble())
        .toList();
            ...
            child: Column(
              children: [
                ...items.asMap().entries.map(
                  (e) => _buildRow(e.value, e.key == items.length - 1),
                ),
                if (items.isEmpty)
                  Padding(
